#!/usr/bin/env python3
"""Upload the NeuTTS Air CPU and ZeroGPU bundles to Hugging Face Spaces."""
...
PROFILES = {
    "cpu": Profile(
        name="CPU",
        default_repo="kaushikpaul/neutts-air-cpu-api",
        files={
    ...
    "zerogpu": Profile(
        name="ZeroGPU",
        default_repo="kaushikpaul/neutts-air-zerogpu",
        files={
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Stage the correct NeuTTS Air files for each hardware profile and "
            "upload them with the authenticated `hf` CLI."
        )
    )
    parser.add_argument(
        "--target",
    ...
    parser.add_argument(
        "--commit-message",
        default="Update NeuTTS Air Space",
...
def upload_profile(
    ...
    with tempfile.TemporaryDirectory(prefix=f"neutts-{profile.name.lower()}-") as temp:
